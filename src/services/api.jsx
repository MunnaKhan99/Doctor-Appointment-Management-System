...
const api = {
  getSpecializations: async () => {
    try {
      ...
      if (Array.isArray(res.data.specializations)) return res.data.specializations;
      console.warn('Unexpected specializations response format:', res.data);
      return [];
    } catch (err) {
  ...
  getDoctors: async ({ search = '', specialization = '', page = 1, limit = 10 }) => {
    try {
      ...
      return {
        doctors: res.data.doctors || res.data.data?.doctors || [],
  ...
  getDoctorAppointments: async ({ status = '', date = '', page = 1 }, token) => {
    try {
      ...
      return {
        appointments: res.data.appointments || res.data.data?.appointments || [],
        totalPages: res.data.totalPages || 1,
        currentPage: res.data.currentPage || page,
      };
    } catch (err) {
