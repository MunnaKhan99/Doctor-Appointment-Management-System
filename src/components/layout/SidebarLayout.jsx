...
const SidebarLayout = ({ children }) => {
  ...
  return (
    <div className="flex h-screen bg-gray-100">
      ...
      <div className={`bg-white shadow-lg transition-all duration-300 hidden lg:block ${isSidebarOpen ? 'w-64' : 'w-16'}`}>
        {/* Sidebar Header */}
        <div className="flex items-center justify-between p-3 lg:p-4 border-b">
          <div className={`flex items-center space-x-2 lg:space-x-3 ${isSidebarOpen ? '' : 'hidden'}`}>
            <span className="text-xl lg:text-2xl">🏥</span>
            <span className="text-lg lg:text-xl font-bold text-gray-900">MediCare</span>
          </div>
          <button
            onClick={() => setIsSidebarOpen(!isSidebarOpen)}
            className="p-1.5 lg:p-2 rounded-md text-gray-500 hover:text-gray-700 hover:bg-gray-100"
          >
          ...
          <div className="px-2 lg:px-3">
            {sidebarItems.map((item) => (
              <Link
                ...
                to={item.path}
                className={`flex items-center px-2 lg:px-3 py-2 mb-1 lg:mb-2 rounded-lg text-xs lg:text-sm font-medium transition-colors ${location.pathname === item.path
              ...
              >
                ...
                {isSidebarOpen && (
                  <span className="ml-2 lg:ml-3 truncate">{item.name}</span>
                )}
              </Link>
            ))}
          </div>
